...
export default class Episode {
    constructor(phrases = [], repeat = 4, off = false, stashed_phrases = []) {
        this.phrases = phrases
        this.repeat = repeat
        this.off = off
        this.stashed_phrases = stashed_phrases
    }
    store() {
        return { repeat: this.repeat.toString(), off: this.off,
            phrases: this.phrases.map(phrase => phrase.store()),
            stashed_phrases: this.stashed_phrases.map(phrase => phrase.store()) }
    }
    load_blocked(loaded) {
        this.off = loaded.off
    ...
    static fromLoaded(loaded, instruments) {
        return new Episode(loaded.phrases.map(loaded_phrase => Phrase.fromLoaded(loaded_phrase, instruments)),
            ...
            (loaded.stashed_phrases || []).map(loaded_phrase => Phrase.fromLoaded(loaded_phrase, instruments)))
    }
    get instruments() {
